...
def get_business_date_from2(_date: date) -> date:
    if is_weekend(_date):
        next_monday_date: date = get_next_monday_from_date(_date)
        return get_business_date_from2(next_monday_date)

    if is_holiday(_date):
        next_day_date: date = get_next_day_from_date(_date)
        return get_business_date_from2(next_day_date)
